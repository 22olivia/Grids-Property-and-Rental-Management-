...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request, ActivityLogger $logger): JsonResponse
    {
        ...
        $logger->log('invoice.created', $invoice, $validated, $request);
    ...
    public function generate(InvoiceService $invoices, ActivityLogger $logger): JsonResponse
    {
        $count = $invoices->generateForActiveLeases();
        $logger->log('invoice.generated_batch', null, ['count' => $count]);
    ...
    public function cancel(Invoice $invoice, ActivityLogger $logger): JsonResponse
    {
        $invoice->update([
            'status' => 'cancelled',
            'remaining_balance' => 0,
        ]);
        $logger->log('invoice.cancelled', $invoice);
    ...
    public function destroy(Invoice $invoice, ActivityLogger $logger): JsonResponse
    {
        $logger->log('invoice.deleted', $invoice, [
