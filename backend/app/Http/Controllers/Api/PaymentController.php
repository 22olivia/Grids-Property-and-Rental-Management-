...
class PaymentController extends Controller
{
    ...
    public function store(Request $request, ActivityLogger $logger): JsonResponse
    {
        ...
        $logger->log('payment.recorded', $payment, $validated, $request);
    ...
    public function approve(Payment $payment, ActivityLogger $logger): JsonResponse
    {
        $payment->update([
        ...
        $payment->invoice?->recalculateBalances();
        $logger->log('payment.approved', $payment);
    ...
    public function reject(Request $request, Payment $payment, ActivityLogger $logger): JsonResponse
    {
        $validated = $request->validate([
            'notes' => ['nullable', 'string'],
        ]);
        ...
        $payment->invoice?->recalculateBalances();
        $logger->log('payment.rejected', $payment, $validated, $request);
    ...
    public function refund(Request $request, Payment $payment, ActivityLogger $logger): JsonResponse
    {
        $validated = $request->validate([
            'amount' => ['required', 'numeric', 'min:0.01'],
        ...
        $payment->invoice?->recalculateBalances();
        $logger->log('payment.refunded', $payment, $validated, $request);
