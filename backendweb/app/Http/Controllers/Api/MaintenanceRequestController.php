...
class MaintenanceRequestController extends Controller
{
    ...
    ): JsonResponse
    {
        $validated = $request->validate([
            ...
            'category' => ['sometimes', Rule::in(DomainCatalog::maintenanceCategories())],
            'priority' => ['sometimes', Rule::in(DomainCatalog::maintenancePriorities())],
        ...
        $maintenanceRequest = MaintenanceRequest::create([
            ...
            'status' => 'open',
            'workflow_status' => $triage ? DomainCatalog::AI_TRIAGED : DomainCatalog::SUBMITTED,
        ...
        try {
            ActivityLog::record('maintenance.created', $maintenanceRequest, [
    ...
    ): JsonResponse
    {
        $validated = $request->validate([
            ...
            'workflow_status' => ['sometimes', Rule::in(DomainCatalog::maintenanceStatuses())],
            'permission_to_enter' => ['sometimes', 'boolean'],
            'preferred_visit_date' => ['nullable', 'date'],
        ...
        if (($validated['status'] ?? null) === 'resolved' && blank($validated['resolved_at'] ?? null)) {
            ...
            $validated['workflow_status'] = $validated['workflow_status'] ?? DomainCatalog::COMPLETED;
        }

        if (($validated['status'] ?? null) === 'in_progress') {
            $validated['resolved_at'] = null;
            $validated['workflow_status'] = $validated['workflow_status'] ?? DomainCatalog::IN_PROGRESS;
        }

        if (! empty($validated['assigned_vendor_id']) || ! empty($validated['assigned_technician_id'])) {
            $validated['workflow_status'] = $validated['workflow_status'] ?? DomainCatalog::ASSIGNED;
        }

        if (! empty($validated['scheduled_date'])) {
            $validated['workflow_status'] = $validated['workflow_status'] ?? DomainCatalog::VISIT_SCHEDULED;
        }

        if (! empty($validated['quotation'])) {
            $validated['workflow_status'] = $validated['workflow_status'] ?? DomainCatalog::QUOTE_SUBMITTED;
        }
        ...
        if (array_key_exists('approved_cost', $validated) && $validated['approved_cost'] !== null) {
            ...
            $validated['workflow_status'] = DomainCatalog::IN_PROGRESS;
        }

        $maintenanceRequest->update($validated);
        $calendar->syncMaintenanceEvent($maintenanceRequest->fresh());
        ActivityLog::record('maintenance.updated', $maintenanceRequest, $validated, $request);
    ...
    {
        ActivityLog::record('maintenance.deleted', $maintenanceRequest, [
            'ticket_number' => $maintenanceRequest->ticket_number,
        ]);
