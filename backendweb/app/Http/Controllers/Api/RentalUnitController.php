...
class RentalUnitController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validated['status'] = DomainCatalog::normalizeUnitStatus($validated['status'] ?? DomainCatalog::VACANT);
        $validated['organization_id'] = $validated['organization_id'] ?? $request->user()->organization_id;

        $unit = RentalUnit::create($validated);
        ActivityLog::record('unit.created', $unit, $validated, $request);
    ...
    public function update(Request $request, RentalUnit $rentalUnit): JsonResponse
    {
        ...
        if (isset($validated['status'])) {
            $validated['status'] = DomainCatalog::normalizeUnitStatus($validated['status']);
        }

        $rentalUnit->update($validated);
        ActivityLog::record('unit.updated', $rentalUnit, $validated, $request);
    ...
    {
        ActivityLog::record('unit.deleted', $rentalUnit, ['unit_number' => $rentalUnit->unit_number]);
        $rentalUnit->delete();
    ...
    public function publishListing(Request $request, RentalUnit $rentalUnit): JsonResponse
    {
        ...
        ActivityLog::record('unit.listing_published', $rentalUnit, $validated, $request);
    ...
    private function validatedPayload(Request $request, bool $partial = false): array
    {
        ...
        return $request->validate([
            ...
            'status' => ['sometimes', Rule::in(DomainCatalog::unitStatuses())],
