...
class AuthController extends Controller
{
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        ActivityLog::record('auth.login', $user, [], $request);
        $token = $user->createToken('api-token')->plainTextToken;
    ...
    public function updateProfile(Request $request): JsonResponse
    {
        ...
        if (isset($validated['email']) && $validated['email'] !== $user->email) {
            $request->validate([
                ...
                'login_email' => ['nullable', 'email', 'max:255', Rule::unique('users', 'email')->ignore($user->id)],
            ]);
        }

        if (array_key_exists('whatsapp_phone', $validated)) {
            $validated['whatsapp_phone'] = preg_replace('/\D+/', '', (string) ($validated['whatsapp_phone'] ?? '')) ?: null;
        }

        if (array_key_exists('notify_email', $validated) && empty($validated['notify_email'])) {
            $validated['notify_email'] = $validated['email'] ?? $user->email;
        }

        if (! Schema::hasColumn('users', 'notify_email')) {
            unset($validated['notify_email']);
        }
        if (! Schema::hasColumn('users', 'whatsapp_phone')) {
            unset($validated['whatsapp_phone']);
        }

        $user->update($validated);
        ActivityLog::record('profile.updated', $user, $validated, $request);
    ...
    public function uploadPhoto(Request $request): JsonResponse
    {
        ...
        ActivityLog::record('profile.photo_updated', $user, ['photo_path' => $path], $request);
    ...
    public function deletePhoto(Request $request): JsonResponse
    {
        ...
        ActivityLog::record('profile.photo_removed', $user, [], $request);
    ...
    public function changePassword(Request $request): JsonResponse
    {
        ...
        ActivityLog::record('profile.password_changed', $user, [], $request);
