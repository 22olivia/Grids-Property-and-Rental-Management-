...
class UserController extends Controller
{
        ...
        $users = User::query()
            ->when($request->search, function ($q) use ($request) {
                ...
                $q->where(function ($inner) use ($term) {
                    ...
                    // Columns added in 2026_08_12_140000 — skip until migrated.
                    if (Schema::hasColumn('users', 'notify_email')) {
                        $inner->orWhere('notify_email', 'like', $term);
                    }
                    if (Schema::hasColumn('users', 'whatsapp_phone')) {
                        $inner->orWhere('whatsapp_phone', 'like', $term);
                    }
                });
            })
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (empty($validated['notify_email'])) {
            $validated['notify_email'] = $validated['email'];
        }
        if (! Schema::hasColumn('users', 'notify_email')) {
        ...
        ActivityLog::record('user.created', $user, ['role' => $user->role], $request);
    ...
    public function update(Request $request, User $user): JsonResponse
    {
        ...
        ActivityLog::record('user.updated', $user, $validated, $request);
    ...
    public function destroy(User $user): JsonResponse
    {
        ...
        ActivityLog::record('user.deleted', $user, ['email' => $user->email]);
        $user->delete();
