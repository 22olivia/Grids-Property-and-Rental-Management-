...
class SupportApiTest extends TestCase
{
    ...
    public function test_staff_can_list_and_assign_maintainer(): void
    {
        $this->seed(DemoRentalSeeder::class);
        ...
        $create = $this->actingAs($tenant, 'sanctum')
            ->postJson('/api/v1/support/tickets', [
                ...
                'email' => $tenant->email,
                'phone' => $tenant->phone,
                'subject' => 'Assign me a maintainer',
                'category' => 'general_query',
                'priority' => 'normal',
                'message' => 'Please reassign this to the property maintainer.',
                'preferred_contact_method' => 'email',
            ]);
        ...
        $this->actingAs($owner, 'sanctum')
            ->getJson('/api/v1/support/maintainers')
            ->assertOk()
            ->assertJsonFragment(['email' => 'manager@grids.test']);

        $this->actingAs($owner, 'sanctum')
            ->postJson('/api/v1/support/tickets/'.$ticketId.'/assign', [
        ...
        $log = \App\Models\ActivityLog::query()
            ->where('action', 'support.ticket_assigned')
            ->latest('id')
            ->first();
        $this->assertNotNull($log);
        $this->assertNull($log->subject_id);
        $this->assertSame($ticketId, $log->properties['subject_key'] ?? null);
    }
}
