...
See **[RAILWAY.md](./RAILWAY.md)** for the full free-tier guide (MySQL plugin, env vars, Vercel `NEXT_PUBLIC_API_URL`).
...
5. On Vercel: `NEXT_PUBLIC_API_URL=https://YOUR-API.up.railway.app/api/v1` → Redeploy.
...
- MySQL plugin → `DB_*` from `${{MySQL.MYSQL*}}`
- `QUEUE_CONNECTION=sync` and `CACHE_STORE=file` on free tier
