...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const wantsFloors = new URL(request.url).searchParams.get('include') === 'floors';
  try {
    return NextResponse.json(
      await withToken(auth.token, () =>
        wantsFloors ? httpAssetRepository.listFloors(id) : httpAssetRepository.getBuilding(id),
      ),
    );
  } catch (error) {
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      await withToken(auth.token, () => httpAssetRepository.updateBuilding(id, body)),
    );
  } catch (error) {
    return toErrorResponse(error);
  }
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  try {
    await withToken(auth.token, () => httpAssetRepository.deleteBuilding(id));
